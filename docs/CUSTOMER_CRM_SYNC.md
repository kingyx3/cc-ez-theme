...
`orders.json?status=<bucket>` has started exhibiting the same failure: a later page can repeat an earlier one's records instead of advancing. Because the Order and ordered-Product stages cannot tolerate a silently incomplete Order bucket the way the Cart stage tolerates a short Checkout snapshot, a repeated page there is not immediately fatal: one single request at a larger `limit` (250, mirroring the Checkout stage's largest tier) is tried first, and an answer shorter than that limit proves it is the whole bucket, exactly as `docs/EASYSTORE_CHECKOUT_CART_SYNC.md` proves a Checkout snapshot complete. Only when that larger request is itself still saturated does the sync give up and fail loudly, because at that point completeness genuinely cannot be proven. This lives in `easystore_hubspot_order_sync._fetch_order_status_bucket`.

`products.json?visibility=<published|unpublished>` can exhibit the exact same repeated-page failure, and did so in production (the Products stage of the 2026-09-26T01:37 UTC run). It gets the identical recovery: one larger-`limit` (250) request is tried before giving up, an answer shorter than that limit proves the visibility bucket complete, and a still-saturated answer fails loudly instead of risking a silently incomplete Product catalogue (which would then also fail the Order stage, since every Order line must resolve to a HubSpot Product). This lives in `easystore_hubspot_products._fetch_product_visibility_bucket`.
