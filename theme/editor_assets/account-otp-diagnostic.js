 ...
 * it never prevents/stops an event, writes an input value, dispatches an event,
 * submits a form, clicks a control, or makes a network request. It records only
 * event metadata and input lengths; OTP digits are never displayed or stored.
 */
(() => {
  ...
  const eventTargetCell = (event) => {
    ...
    return index < 0 ? null : { target, index, all };
  };
  ...
  window.addEventListener('beforeinput', describe, true);
  window.addEventListener('input', describe, true);
