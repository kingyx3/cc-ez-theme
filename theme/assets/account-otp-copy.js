 ...
 *
 * Temporary preview diagnostic: opening a preview once with ?otpdiag=1 enables
 * a session-scoped, read-only beforeinput probe. It records event metadata and
 * input counts only; it never records OTP digits or changes browser behaviour.
 */
(() => {
  ...
  const startBeforeInputDiagnostic = () => {
    ...
    show([
      ...
      'Need: beforeinput + cancelable=true + dataLength=6'
    ]);
    ...
    window.addEventListener('beforeinput', (event) => {
      ...
      const looksLikeFullCode = dataLength === 6;
      const safeCandidate = looksLikeFullCode && event.cancelable === true && event.isTrusted === true;

      show([
        `OTP DIAG: ${safeCandidate ? 'SAFE CANDIDATE' : 'OBSERVED'}`,
        `beforeinput: yes`,
        ...
        safeCandidate
          ? 'Result: prerequisite met for guarded interception.'
          : 'Result: do NOT intercept unless cancelable=true and dataLength=6.'
      ]);
    }, true);
  };
