...
class LastTrackedTouchTests(unittest.TestCase):
    def test_latest_touch_before_order_wins(self) -> None:
        order_at = 100 * DAY
        touches = [
            {"click_id": "old", "clicked_at": 80 * DAY, "bound_at": 80 * DAY},
            {"click_id": "winner", "clicked_at": 99 * DAY, "bound_at": 99 * DAY},
            {"click_id": "after", "clicked_at": 101 * DAY, "bound_at": 101 * DAY},
        ]
        self.assertEqual(
    ...
    def test_touch_bound_after_order_cannot_retroactively_claim_it(self) -> None:
        order_at = 100 * DAY
        touches = [
            {"click_id": "late-bind", "clicked_at": 99 * DAY, "bound_at": 101 * DAY}
        ]
        self.assertIsNone(
            attribution.latest_touch_for_order(touches, order_at=order_at, window_days=30)
        )
    ...
    def test_touch_outside_window_is_not_used(self) -> None:
        order_at = 100 * DAY
        touches = [
            {"click_id": "too-old", "clicked_at": 69 * DAY, "bound_at": 69 * DAY}
        ]
        self.assertIsNone(
            attribution.latest_touch_for_order(touches, order_at=order_at, window_days=30)
        )
...
class SyncTests(unittest.TestCase):
    ...
    def test_existing_different_order_click_is_never_overwritten(self) -> None:
        ...
        self.assertEqual(summary["hubspot_orders_with_conflicting_click_id"], 1)
